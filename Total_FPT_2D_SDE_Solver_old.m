function [LE,LI,LEmax,LImax,M1s_val,M2s_val,Ds_val,rho_o,rho_ln_o,v_o] = Total_FPT_2D_SDE_Solver_old(v_o,rho_o,rho_ln_o,S_EE,S_IE,S_EI,S_II,NE,NI,N_neuron_fire_first)
N_sample = 10000;
vT =1;vI = -2/3;
...
% hist2d([x'; y'],xdivided,ydivided,[xs,xe],[ys,ye]);
mhist = hist2d([x'; y'],xdivided,ydivided,[xs,xe],[ys,ye]);
% % % size(xdivided),size(ydivided),size(mhist),
% % % pause;
...
% % % LEmax,
% % % LImax,
% hold on; plot(ycord(1:end-1),s_val,'g','LineWidth',2);
elseif count1>1
y = All_phi(:,1); x = All_phi(:,2);
ttIdx = find(tt<y);
h = tt(2) -tt(1);
